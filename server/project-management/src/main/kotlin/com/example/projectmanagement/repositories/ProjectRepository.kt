...
interface ProjectRepository : JpaRepository<Project, Long> {
    ...
    @EntityGraph(value = "Project.withDetails", type = EntityGraph.EntityGraphType.LOAD) // или FETCH
    ...
    @EntityGraph(value = "Project.withDetails", type = EntityGraph.EntityGraphType.LOAD) // или FETCH
    // Можно оставить findById, Spring Data JPA должен подхватить EntityGraph для стандартных методов, если он один,
    // но явный @Query с @EntityGraph надежнее.
    @Query("SELECT p FROM Project p WHERE p.id = :projectId")
    fun findProjectByIdWithDetails(@Param("projectId") projectId: Long): Optional<Project>

    // Стандартные методы findByOwnerId и existsByIdAndOwner_Id остаются без изменений
